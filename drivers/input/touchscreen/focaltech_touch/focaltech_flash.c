...
u8 FTS_FW_READ = 0;     //add by shenwenbin for factory show firmware 20190322
...
static u32 fd_support = false;
void static my_memcopy(void* d, void* s, u32 n){
    u32 num = n >> 3;
    u32 tail = n&0x07;

    u32 i = num;
    while(i--){ ((u64*)d)[i] = ((u64*)s)[i]; }
    if(tail){
        d = &(((u64*)d)[num]);
        s = &(((u64*)s)[num]);

        i = tail;
        while(i--){ ((u8*)d)[i] = ((u8*)s)[i]; }
    }
}
static int fts_pram_write(u32 saddr, const u8 *buf, u32 len)
{
    ...
    packet_size = (fd_support?
                   FTS_FLASH_PACKET_LENGTH_SPI:
                   FTS_FLASH_PACKET_LENGTH_SPI_LOW);

    if(NULL == fw_buf){
        fw_buf = kzalloc(packet_size + FTS_CMD_WRITE_LEN, GFP_KERNEL);
        if (NULL == fw_buf) {
    ...
    if (remainder > 0){packet_number++;}
    packet_len = packet_size;

    fw_buf[0] = FTS_ROMBOOT_CMD_WRITE;
    for (i = 0; i < packet_number; i++) {
        offset = i * packet_size;
        addr = saddr + offset;
        fw_buf[1] = BYTE_OFF_16(addr);
        fw_buf[2] = BYTE_OFF_8(addr);
        fw_buf[3] = BYTE_OFF_0(addr);

        /* last packet */
        if ((i == (packet_number - 1)) && remainder){packet_len = remainder;}
        fw_buf[4] = BYTE_OFF_8(packet_len);
        fw_buf[5] = BYTE_OFF_0(packet_len);

        my_memcopy((void*)(fw_buf + FTS_CMD_WRITE_LEN), (void*)(buf + offset), packet_len);
        ret = fts_write(&fw_buf[0], FTS_CMD_WRITE_LEN + packet_len);
        if (ret < 0) {
...
#endif
    /*modify by shenwenbin for TP self test can't touch 20190428 begin */ 
    fts_irq_enable();
    /*modify by shenwenbin for TP self test can't touch 20190428 end */
...
/*add by shenwenbin for factory show firmware 20190322 begin*/
#ifdef SUPPORT_READ_TP_VERSION
 static void fts_fw_ver(struct fts_ts_data *data)
{
	int err;
        u8 reg_addr;
	reg_addr = FTS_REG_FW_VER;

        err = fts_read_reg(reg_addr, &data->fw_ver);
	if (err < 0)
		FTS_ERROR("fw version read failed\n");
}
#endif
/*add by shenwenbin for factory show firmware 20190322 end*/
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    #ifdef SUPPORT_READ_TP_VERSION	
        if(!FTS_FW_READ){
                fts_fw_ver(upg->ts_data);
                memset(fw_version, 0, sizeof(fw_version));
                sprintf(fw_version, "[FW]0x%02x,[IC]FT8719", upg->ts_data->fw_ver);
