...
#define LCD_ESD_PATCH                   1       /*modify by shenwenbin for LCD ESD check need use TP read status 20190428*/
...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    ...
    //FTS_DEBUG("swb.%s lcd_need_reset = %d ;tp_need_recovery = %d\n",__func__,lcd_need_reset,tp_need_recovery);

    if((tp_need_recovery == 0) && (lcd_need_reset == 0)) {
	ret = fts_read_reg(FTS_REG_ESD_SATURATE, &val);
		...
		return -EIO;
            ...
            if (val == 0xAA) {
                ...
                lcd_need_reset = 1;
                tp_need_recovery = 1;
            FTS_DEBUG("swb.%s lcd_need_reset = %d ;tp_need_recovery = %d",__func__,lcd_need_reset,tp_need_recovery);    
            }
    }
