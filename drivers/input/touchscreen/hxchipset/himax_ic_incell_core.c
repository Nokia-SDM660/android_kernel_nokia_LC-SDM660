...
static int himax_mcu_flash_write_burst_lenth(uint8_t *reg_byte, uint8_t *write_data, uint32_t length)
{
	...
	data_byte = kzalloc(sizeof(uint8_t)*(length + ADDR_LEN_4), GFP_KERNEL);
...
static void hx_dis_rload_0f(int disable)
{
	/*Diable Flash Reload*/
	g_core_fp.fp_register_write (pzf_op->addr_dis_flash_reload, DATA_LEN_4, pzf_op->data_dis_flash_reload, 0);
}

static void himax_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	int total_read_times = 0;
...
static void himax_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
	int total_read_times = 0;
	int max_bus_size = MAX_I2C_TRANS_SZ;
	int total_size_temp = 0;
...
static int himax_sram_write_crc_check(const struct firmware *fw_entry, uint8_t *addr, int strt_idx, uint32_t len)
{
	int retry = 0;
	int crc = -1;
...
static int himax_zf_part_info(const struct firmware *fw_entry)
{
	int part_num = 0;
	int ret = 0;
	uint8_t buf[16];
...
static void himax_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
	int ret = 0;
	...
	/* set n frame=0*/
        /*modify by shenwenbin for himax FAE requeset 20190603 begin*/
	if (G_POWERONOF == 1) {
        ...
        /*modify by shenwenbin for himax FAE requeset 20190603 end*/

	I("%s, End \n", __func__);
}

static int hx_0f_op_file_dirly(char *file_name)
{
	int err = NO_ERR;
...
static int himax_mcu_0f_operation_dirly(void)
{
	int err = NO_ERR;
	const struct firmware *fw_entry = NULL;
...
static void himax_mcu_0f_operation(struct work_struct *work)
{
	int err = NO_ERR;
	const struct firmware *fw_entry = NULL;
        /*add by shenwenbin for factory show firmware 20190329 begin*/
        #ifdef SUPPORT_READ_TP_VERSION
        char fw_version[64];
        #endif
        /*add by shenwenbin for factory show firmware 20190329 end*/
	...
	g_core_fp.fp_read_FW_ver();
        ...
        sprintf(fw_version, "[FW]0x%04x,0x%04x,[IC]HX83112A", ic_data->vendor_fw_ver,ic_data->vendor_config_ver);
        init_tp_fm_info(0, fw_version, "Himax");
        #endif
        /*add by shenwenbin for factory show firmware 20190329 end*/
	g_core_fp.fp_touch_information();
	msleep (10);
...
#ifdef HX_0F_DEBUG
static void himax_mcu_read_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, int read_len)
{
	int total_read_times = 0;
	int max_bus_size = MAX_I2C_TRANS_SZ;
	int total_size_temp = 0;
...
static void himax_mcu_read_all_sram(uint8_t *addr, int read_len)
{
	int total_read_times = 0;
	int max_bus_size = MAX_I2C_TRANS_SZ;
	int total_size_temp = 0;
...
static void himax_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	uint8_t tmp_addr[4];
...
static void himax_mcu_0f_operation_check(int type)
{
	int err = NO_ERR;
	const struct firmware *fw_entry = NULL;
